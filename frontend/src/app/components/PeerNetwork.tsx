    ...
    <div className="max-w-6xl mx-auto py-8 space-y-16 text-foreground">
      ...
      <div className="text-center relative">
        ...
        <motion.div 
          ...
          className="inline-flex items-center gap-2 px-4 py-1.5 bg-muted rounded-full border border-border text-[10px] font-bold text-indigo-400 tracking-[0.3em] mb-6"
        >
          <Users className="w-3.5 h-3.5" />
          Neural Collaboration Grid
        </motion.div>
        <h2 className="text-4xl md:text-7xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-foreground via-foreground to-foreground/50 leading-none mb-6">
          Peer Network
        </h2>
        <p className="text-muted-foreground max-w-xl mx-auto font-bold text-[10px] leading-loose">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 pb-20">
        ...
        <div className="lg:col-span-8 space-y-10">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold tracking-[0.4em] text-foreground/60 flex items-center gap-4">
            ...
            {ALUMNI.map((alumni) => (
              <motion.div key={alumni.name} variants={itemVariants}>
                <Card className="p-8 bg-card border-border hover:border-indigo-500/50 transition-all duration-500 group relative overflow-hidden h-full flex flex-col rounded-[32px] shadow-2xl">
                  ...
                  <div className="relative z-10 flex-1">
                    <div className="flex items-start gap-5 mb-8">
                      <div className={`w-16 h-16 rounded-[24px] bg-gradient-to-br ${alumni.color} p-[1px]`}>
                        <div className="w-full h-full rounded-[23px] bg-card flex items-center justify-center text-2xl font-bold text-foreground">
                          {alumni.name.charAt(0)}
                        </div>
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          <h4 className="text-lg font-bold text-foreground">{alumni.name}</h4>
                          <span className="text-[10px] font-bold text-muted-foreground">{alumni.year}</span>
                        </div>
                        <p className="text-[11px] text-indigo-400 font-bold">{alumni.uni}</p>
                        <p className="text-[10px] text-muted-foreground font-bold mt-1">{alumni.course}</p>
                      </div>
                    </div>

                    <div className="flex flex-wrap gap-2 mb-10">
                      {alumni.tags.map(tag => (
                        <span key={tag} className="px-3 py-1 rounded-full bg-muted border border-border text-[9px] font-bold text-foreground/50 group-hover:border-indigo-500/30 group-hover:text-foreground transition-colors">
                          {tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <Button 
                    className="w-full h-14 bg-muted hover:bg-indigo-600 border border-border hover:border-indigo-500 text-foreground font-bold tracking-[0.2em] text-[10px] rounded-2xl transition-all duration-500 group/btn"
        ...
        <div className="lg:col-span-4 space-y-10">
          <div className="flex items-center gap-4">
            <h3 className="text-sm font-bold tracking-[0.4em] text-foreground/60">
              Active Squads
            </h3>
          ...
          <div className="space-y-6">
                ...
                <Card className="p-6 bg-muted/50 border-border/50 hover:border-indigo-500/30 cursor-pointer group transition-all duration-500 rounded-[28px] relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-5 relative z-10">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-xs font-bold text-foreground group-hover:text-indigo-400 transition-colors">{squad.name}</h4>
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-[10px] text-muted-foreground font-bold">{squad.members} Active Users</span>
                        <span className={`text-[9px] font-bold ${squad.color}`}>{squad.status}</span>
                      </div>
                    </div>
                    <Button 
                      size="icon" 
            ...
            <Button 
              variant="outline" 
              className="w-full h-14 border-dashed border-border bg-transparent text-muted-foreground hover:border-indigo-500/50 hover:text-indigo-400 font-bold tracking-[0.2em] text-[10px] rounded-[24px] transition-all group"
            >
          ...
          <Card className="p-8 bg-indigo-600/10 border border-indigo-500/20 rounded-[32px] relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <h4 className="text-sm font-bold text-foreground mb-2">Earn Recognition</h4>
              <p className="text-[10px] text-foreground/50 font-bold leading-relaxed">
