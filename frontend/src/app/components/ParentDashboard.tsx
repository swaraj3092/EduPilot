    ...
    <div className="max-w-6xl mx-auto py-8 space-y-16 text-foreground">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 relative">
        ...
        <div className="space-y-4 relative z-10">
          ...
          <h2 className="text-4xl md:text-7xl font-black italic tracking-tighter uppercase bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-white/20 leading-none">
            Parent Hub
          </h2>
          <p className="text-white/40 max-w-lg font-bold uppercase text-[10px] tracking-widest leading-loose">
        ...
        <Button 
          className="bg-rose-600 hover:bg-rose-500 text-white font-black uppercase tracking-[0.2em] rounded-2xl shadow-2xl shadow-rose-500/30 h-16 px-10 group relative overflow-hidden w-full lg:w-auto transition-all active:scale-95 border-t border-white/20"
          onClick={handleShare}
        >
      ...
      >
        <motion.div variants={itemVariants}>
          <Card className="p-8 bg-white/[0.03] border-white/10 shadow-2xl rounded-[32px] flex flex-col items-center text-center group relative overflow-hidden hover:border-indigo-500/50 transition-all duration-500">
            ...
            <h4 className="font-black uppercase tracking-[0.3em] text-[10px] text-white/30 mb-2">Live Target Count</h4>
            <p className="text-5xl font-black text-white italic tracking-tighter">{apps.length}</p>
            <p className="text-[10px] text-indigo-400 uppercase font-black tracking-[0.2em] mt-3">University Protocols</p>
          </Card>
        </motion.div>
        
        <motion.div variants={itemVariants}>
          <Card className="p-8 bg-white/[0.03] border-white/10 shadow-2xl rounded-[32px] flex flex-col items-center text-center group relative overflow-hidden hover:border-emerald-500/50 transition-all duration-500">
            ...
            <h4 className="font-black uppercase tracking-[0.3em] text-[10px] text-white/30 mb-2">Journey Completion</h4>
            <p className="text-5xl font-black text-white italic tracking-tighter">{overallProgress}%</p>
            <p className="text-[10px] text-emerald-400 uppercase font-black tracking-[0.2em] mt-3">Objective Velocity</p>
          </Card>
        </motion.div>

        <motion.div variants={itemVariants}>
          <Card className="p-8 bg-white/[0.03] border-white/10 shadow-2xl rounded-[32px] flex flex-col items-center text-center group relative overflow-hidden hover:border-orange-500/50 transition-all duration-500">
            ...
            <h4 className="font-black uppercase tracking-[0.3em] text-[10px] text-white/30 mb-2">Fiscal Readiness</h4>
            <p className="text-5xl font-black text-white italic tracking-tighter">Gold</p>
      ...
      >
        <Card className="p-10 bg-white/[0.02] border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.5)] relative overflow-hidden rounded-[40px]">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-12 relative z-10">
            <h3 className="text-xl font-black uppercase tracking-[0.3em] text-white flex items-center gap-4 italic">
              <Calendar className="w-7 h-7 text-indigo-500" />
              Active Milestone Feed
            </h3>
            <div className="px-5 py-2 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white/40 flex items-center gap-2">
          ...
          <div className="space-y-12 relative z-10 ml-4">
            {apps.length > 0 ? apps.map((app: any, i: number) => (
              <motion.div 
                ...
                className="relative pl-12 border-l-2 border-white/5 pb-10 last:pb-0"
              >
                 ...
                 <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 p-6 bg-white/[0.02] border border-white/5 rounded-[24px] hover:bg-white/[0.04] transition-all group">
                    <div className="space-y-2">
                      <div className="flex items-center gap-3">
                        <h4 className="font-black text-xl text-white uppercase tracking-tighter italic">{app.university}</h4>
                        {app.status === 'submitted' && <CheckCircle2 className="w-4 h-4 text-emerald-400" />}
                      </div>
                      <p className="text-[10px] text-white/30 font-black uppercase tracking-widest">{app.program}</p>
                    </div>
                    
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 w-full lg:w-auto">
                      <div className="flex-1 lg:w-48">
                         <div className="flex justify-between mb-2">
                            <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">Protocol Stage</span>
                            <span className="text-[9px] font-black text-white/60 uppercase tracking-widest">{app.status === 'submitted' ? '100%' : '45%'}</span>
                         </div>
                         <Progress value={app.status === 'submitted' ? 100 : 45} className={`h-1.5 ${app.status === 'submitted' ? 'bg-emerald-500/20' : 'bg-white/5'}`} />
                      </div>
                      <div className={`text-[9px] font-black px-6 py-2 rounded-xl uppercase tracking-[0.25em] transition-all duration-500 ${app.status === 'submitted' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-white/5 text-white/20 border border-white/10'}`}>
              ...
              <div className="text-center py-24 bg-white/[0.02] rounded-[40px] border-2 border-dashed border-white/5">
                 <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mx-auto mb-6">
                    <Target className="w-8 h-8 text-white/10" />
                 </div>
                 <p className="text-white/20 font-black uppercase tracking-[0.3em] text-xs">No active protocols detected.</p>
              </div>
            )}
          </div>
        </Card>
